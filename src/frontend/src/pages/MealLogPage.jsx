...
const MealLogPage = () => {
  ...
  // --- STATES CHO TÍNH NĂNG THÊM MÓN THÔNG MINH ---
  ...
  const [manualMeal, setManualMeal] = useState({
    mealId: '', 
    foodName: '', 
    mealType: 'Bữa sáng', // Giữ nguyên "Bữa sáng" để không bị lỗi 500 Database
    servings: 1, 
  ...
  const handleDeleteLog = async (logId, foodName) => {
    if (!window.confirm(`Bạn muốn xóa "${foodName}" khỏi nhật ký hôm nay?`)) return;
    try {
      const token = localStorage.getItem('nutrifood_token');
      await axios.delete(`http://localhost:5001/api/meal-logs/${logId}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      ...
      <main className="flex-grow pt-28 pb-20 px-4">
        <div className="max-w-7xl mx-auto">
          <div className="mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
            <div>
              <h1 className="text-3xl font-black text-slate-900 tracking-tight">Bảng Điều Khiển Dinh Dưỡng</h1>
       ...
       <div className="bg-white rounded-2xl p-2 shadow-sm border border-slate-100 flex items-center gap-2">
        <input 
          ...
          className="bg-slate-50 border border-slate-200 text-slate-800 font-bold rounded-xl px-4 py-2 focus:outline-none focus:border-green-500 [&::-webkit-calendar-picker-indicator]:brightness-0 [&::-webkit-calendar-picker-indicator]:cursor-pointer"
        />
        <button 
    onClick={() => setSelectedDate(todayStr)}
    className={`px-4 py-2 font-bold rounded-xl transition-colors ${selectedDate === todayStr ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
        >
    Hôm nay
        </button>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
            {[
              ...
              <div key={idx} className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 text-center">
                <p className="text-slate-500 text-sm font-bold mb-1">{item.label}</p>
                <p className={`text-2xl font-black text-${item.color}-500`}>{Math.round(item.value)}</p>
          ...
          <div className="grid lg:grid-cols-2 gap-8">
            <div className="bg-gradient-to-b from-green-50 to-white rounded-[2rem] border border-green-100 p-8 flex flex-col justify-center items-center text-center">
              ...
              <h2 className="text-xl font-black text-green-800 mb-2">Lộ trình AI</h2>
              ...
              <button onClick={() => navigate('/meal-plan')} className="bg-green-600 text-white font-bold px-6 py-3 rounded-xl hover:bg-green-700 shadow-md">Khám phá Lộ trình AI</button>
            </div>

            <div className="bg-white rounded-[2rem] border border-slate-100 p-6 shadow-sm">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-black text-slate-800">📝 Thực Tế Đã Ăn</h2>
                <button onClick={() => setIsAddModalOpen(true)} className="bg-slate-100 text-slate-700 font-bold px-4 py-2 rounded-xl hover:bg-slate-200 transition">+ Thêm Món</button>
              </div>
                ...
                <div className="space-y-4">
                  {logs.map(log => (
                    <div key={log._id} className="bg-slate-50 p-4 rounded-2xl flex items-center justify-between border border-slate-100">
                      ...
                      <button onClick={() => handleDeleteLog(log._id, log.foodName)} className="text-slate-400 hover:text-rose-500 p-2 shrink-0">
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm animate-fadeIn">
          <div className="bg-white rounded-[2rem] w-full max-w-lg shadow-2xl overflow-hidden">
            <div className="p-6 border-b border-slate-100 flex justify-between items-center">
              <h3 className="text-xl font-black text-slate-900">Thêm Món Ăn</h3>
            ...
            <form onSubmit={handleManualSubmit} className="p-6 space-y-5 bg-slate-50">
              ...
              <div className="bg-white p-4 rounded-2xl border border-green-100 shadow-sm">
                <label className="block text-xs font-bold text-green-600 uppercase tracking-wider mb-2">🔍 Tra cứu món ăn từ Hệ thống</label>
                <select 
                  value={manualMeal.mealId} 
                  onChange={handleMealSelection} 
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold focus:border-green-500 outline-none cursor-pointer"
                >
              ...
              <div className="grid grid-cols-5 gap-4">
                <div className="col-span-3">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Tên món ăn <span className="text-rose-500">*</span></label>
                  <input required type="text" value={manualMeal.foodName} onChange={(e) => setManualMeal({...manualMeal, foodName: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold focus:border-green-500 outline-none" placeholder="VD: Bánh mì chả lụa" />
                </div>
                <div className="col-span-2">
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Bữa ăn</label>
                  <select value={manualMeal.mealType} onChange={(e) => setManualMeal({...manualMeal, mealType: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold focus:border-green-500 outline-none">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Số phần ăn (Servings)</label>
                  <input type="number" min="0.5" step="0.5" value={manualMeal.servings} onChange={handleServingsChange} className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 text-sm font-bold focus:border-green-500 outline-none" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Tổng Calories (kcal)</label>
                  <input type="number" min="0" value={manualMeal.calories} onChange={(e) => setManualMeal({...manualMeal, calories: e.target.value})} className="w-full bg-orange-50 border border-orange-200 text-orange-700 rounded-xl px-4 py-3 text-sm font-bold focus:border-orange-500 outline-none" placeholder="0" />
                </div>
              </div>

              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Pro (g)</label>
                  <input type="number" min="0" value={manualMeal.protein} onChange={(e) => setManualMeal({...manualMeal, protein: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold focus:border-green-500 outline-none" placeholder="0" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Carb (g)</label>
                  <input type="number" min="0" value={manualMeal.carbs} onChange={(e) => setManualMeal({...manualMeal, carbs: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold focus:border-green-500 outline-none" placeholder="0" />
                </div>
                <div>
                  <label className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1">Fat (g)</label>
                  <input type="number" min="0" value={manualMeal.fat} onChange={(e) => setManualMeal({...manualMeal, fat: e.target.value})} className="w-full bg-white border border-slate-200 rounded-xl px-3 py-2 text-sm font-bold focus:border-green-500 outline-none" placeholder="0" />
                </div>
              </div>

              <div className="pt-2 mt-4 flex gap-3">
                <button type="button" onClick={() => setIsAddModalOpen(false)} className="flex-1 px-4 py-3 bg-white border border-slate-200 text-slate-600 font-bold rounded-xl hover:bg-slate-50">Hủy</button>
                <button type="submit" className="flex-1 px-4 py-3 bg-slate-900 text-white font-bold rounded-xl hover:bg-green-600 transition-colors">Lưu món vào nhật ký</button>
              </div>
            </form>
          </div>
        </div>
